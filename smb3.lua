...
-- MODIFIED by @or4ng33xp0 to add Mario (x,y), collision-related pixels, and HB (x,y)

--toggle features, change to false if you don't want them
local toggle_display_pixel_boost                    = false
...
local toggle_display_mario_position                 = false
local toggle_display_mario_velocity                 = false
local toggle_display_p_meter                        = false
local toggle_display_next_p                         = false
local toggle_display_p_kill_counter                 = false
local toggle_display_block_y                        = false --for walljumps
local toggle_display_rerecords                      = false
local toggle_display_lag_frames                     = false
local toggle_display_frames                         = false
local toggle_display_level                          = false
...
local mario_pos_line_color     = "yellow"
...
local ram_slope_en         = 0x0563 --are slopes enabled?
local ram_Map_Object_Data  = 0x052d --map object data array
local ram_Map_Object_ActY  = 0x0500 --map objects' actual Y positions
local ram_Map_Object_ActX  = 0x050f --map objects' actual X positions
local ram_Map_Object_ActXH = 0x051e --map objects' actual XHi positions
local ram_Map_Objects_IDs  = 0x7f15 --map object IDs
local ram_Map_Operation    = 0x0729 --current map operation
local ram_World_Num        = 0x0727 --current world
...
local rom_offs_sloped_tbl   = 0x11396 --sloped level offset table
local rom_offs_flat_tbl     = 0x11356 --flat level offset table
...
function display_mario_x_y()
    local mario_x = (memory.readbyte(ram_high_x) * 0x100) + memory.readbyte(ram_x)
    local mario_y = (memory.readbyte(ram_high_y) * 0x100) + memory.readbyte(ram_y)
    local mario_pos_x = mario_x - screen[0]
    local mario_pos_y = mario_y - screen[2]
    gui.line(mario_pos_x, 0, mario_pos_x, screen_height*16, mario_pos_line_color)
    gui.line(0, mario_pos_y, screen_width*16, mario_pos_y, mario_pos_line_color)
    
    local tbladdr = (memory.readbyte(ram_slope_en) and rom_offs_sloped_tbl or rom_offs_flat_tbl)
    tbladdr = tbladdr + 6 -- start at "in-front, upper"
    if (memory.readbyte(ram_mario_suit) == 0) or (memory.readbyte(ram_is_crouching) == 1) then
        tbladdr = tbladdr + 16 -- if we're small, offset into the small table
    end
    for i=1, 4 do
        local x = mario_pos_x + rom.readbyte(tbladdr+1)
        local y = mario_pos_y + rom.readbyte(tbladdr)
        gui.pixel(x, y, "white")
        tbladdr = tbladdr - 2
    end
end
...
function display_bro_movement(objindex, y_counter)
    --local world = memory.readbyte(ram_World_Num) + 1
    --if world ~= 2 then -- We only care about World 2
    --    return
    --end

    if memory.readbyte(ram_Map_Operation) == 7 then -- (7 = MO_DoLevelClear)
        -- During MO_DoLevelClear, we can look at which way the bros are facing
        local face = (memory.readbyte(ram_Map_Object_Data + objindex) == 0) and "(R)" or "(L)"
        gui.drawtext(160, y_counter, face, text_color, text_back_color)
    end

    if memory.readbyte(ram_Map_Operation) == 0xB then -- (0xB = MO_HammerBroMarch)
        -- During MO_HammerBroMarch, we can look at which way the bros are moving
        oneshot = false
        local move = memory.readbyte(ram_Map_Object_Data + objindex)
        local movearr = {
            "Move RIGHT",
            "Move LEFT",
            "Move DOWN",
        ...
        local movestr = movearr[move+1]
        gui.drawtext(160, y_counter, movestr, text_color, text_back_color)
    end
...
function display_map_objs()
    ...
    -- local ram_Map_Object_ActX  = 0x050f --map objects' actual X positions
    -- local ram_Map_Object_ActXH = 0x051e --map objects' actual XHi positions
    if memory.readbyte(ram_tileset) ~= 0 then
        return
    end
    ...
    for i=1, 14 do
        local objid = memory.readbyte(ram_Map_Objects_IDs + (i-1))
        local x = memory.readbyte(ram_Map_Object_ActX + (i-1))
        local xhi = memory.readbyte(ram_Map_Object_ActXH + (i-1))
        local y = memory.readbyte(ram_Map_Object_ActY + (i-1))
        if (objid ~= 0) then
            gui.drawtext(1, y_counter, string.format("%d:%X", i-1, objid), text_color, text_back_color) --display sprite id
            gui.drawtext(17, y_counter, string.format("(%02X, %02X)", xhi*256+x, y), text_color, text_back_color) --draw position
            gui.drawtext(65, y_counter, objstr[objid+1], text_color, text_back_color)
            if (objid >= 3) and (objid <= 6) then -- for bros, display movement
                display_bro_movement(i-1, y_counter)
            end
            y_counter = y_counter + 8 --add to y_counter so the next sprite is shown below the previous
        end
    end
...
function postframe_calculations()
    ...
    if toggle_display_map_objs then
        display_map_objs()
    end
